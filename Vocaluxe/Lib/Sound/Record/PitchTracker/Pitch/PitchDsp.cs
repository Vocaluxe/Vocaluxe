...
namespace Vocaluxe.Lib.Sound.Record.PitchTracker.Pitch
{
    ...
    public class PitchDsp
    {
        ...
        public PitchDsp(double sampleRate, float minPitch, float maxPitch)
        {
            ...
            double freqStep = 1.0 / Math.Pow(2.0, 1.0 / kCourseOctaveSteps);
        ...
        private float DetectPitchLo(float[] samplesLo, float[] samplesHi)
        {
            ...
            for (int idx = 0; idx < m_numCourseSteps; idx += skipSize)
            {
                ...
                int stepSizeLoRes = blockLen / 10;
                int stepSizeHiRes = Math.Max(1, Math.Min(5, idx * 5 / m_numCourseSteps));
                ...
                if (fValue > peakThresh1)
                {
                    ...
                    int begSearch = Math.Max(idx - peakScanSizeHalf, 0);
                    int endSearch = Math.Min(idx + peakScanSizeHalf, m_numCourseSteps - 1);

                    while (curPos >= begSearch && curPos < endSearch)
                    {
                        float curVal = RatioAbsDiffLinear(curSamples, curPos, blockLen, stepSizeHiRes, true);
                        ...
                        if (prevVal > curVal)
                        {
                            ...
                            if (dir == 0)
                            {
                                ...
                                {
                                    float fValL = RatioAbsDiffLinear(curSamples, peakIdx - 5, blockLen, stepSizeHiRes, true);
                                    float fValR = RatioAbsDiffLinear(curSamples, peakIdx + 5, blockLen, stepSizeHiRes, true);
                                    float fPointy = peakVal / (fValL + fValR) * 2.0f;

                                    float minPointy = (m_prevPitchIdx > 0 && Math.Abs(m_prevPitchIdx - peakIdx) < 10) ? 1.2f : 1.5f;

                                    if (fPointy > minPointy)
                                    {
                                        float pitchHi = DetectPitchHi(curSamples, peakIdx);
        ...
        private float DetectPitchHi(float[] samples, int lowFreqIdx)
        {
            ...
            while (curPos >= 0 && curPos < kScanHiSize)
            {
                ...
                if (prevVal > m_peakBuf[curPos])
                {
                    ...
                    {
                        // found the peak
                        float minVal = Math.Min(m_peakBuf[peakIdx - 1], m_peakBuf[peakIdx + 1]);
                        ...
                        float fIdx = (float)peakIdx + (y3 - y1) / (2.0f * (2.0f * y2 - y1 - y3));
        ...
        {
            double angleStep = freq / sampleRate * Math.PI * 2.0;
        ...
        public bool LevelIsAbove(float[] buffer, int len, float level)
        {
            ...
            int endIdx = Math.Min(buffer.Length, len);
        ...
        {
            float c1 = 0.5f * (fY2 - fY0);
            float c3 = 1.5f * (fY1 - fY2) + 0.5f * (fY3 - fY0);
            float c2 = fY0 - fY1 + c1 - c3;
        ...
        private float RatioAbsDiffLinear(float[] samples, int freqIdx, int blockLen, int stepSize, bool hiRes)
        {
            ...
            float offsetFrac = m_pCourseFreqOffset[freqIdx] - offsetInt;
            float rect = 0.0f;
            float absDiff = 0.01f; // prevent divide by zero
            int count = 0;
            float interp;
            float sample;
            ...
            float finalVal = rect / absDiff * 100.0f;
        ...
        private float SumAbsDiffHermite(float[] samples, float fOffset, int blockLen, int stepSize)
        {
            ...
            float offsetFrac = fOffset - offsetInt;
            float value = 0.001f; // prevent divide by zero
            int count = 0;

            // do a scan using linear interpolation and the specified step size
            for (int idx = 0; idx < blockLen; idx += stepSize, count++)
            {
                int offsetIdx = offsetInt + idx;
        ...
        public static bool PitchToMidiNote(float pitch, out int note, out int cents)
        {
            ...
            float fNote = (float)((12.0 * Math.Log10(pitch / 55.0) * InverseLog2)) + 33.0f;
            note = (int)(fNote + 0.5f);
        ...
        public float MidiNoteToPitch(float note)
        {
            ...
            float pitch = (float)Math.Pow(10.0, (note - 33.0f) / InverseLog2 / 12.0f) * 55.0f;
