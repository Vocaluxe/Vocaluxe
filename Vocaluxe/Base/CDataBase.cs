...
namespace Vocaluxe.Base
{
    static class CDataBase
    {
        ...
        private static bool GetDataBaseSongInfos(int SongID, out string Artist, out string Title, out int NumPlayed, out string DateAdded, string FilePath)
        {
            Artist = String.Empty;
            Title = String.Empty;
            NumPlayed = 0;
        ...
        private static bool UpdateDatabase(int CurrentVersion, SQLiteConnection connection)
        {
            ...
            if (CurrentVersion < 3)
            {
                updated &= ConvertV2toV3(connection);
            }
            return updated;
        }
        ...
        private static bool ImportData(string SourceDBPath, string DestinationDBPath)
        {
            ...
            while (rSource.Read())
            {
                ...
                int numplayed;
                if (GetDataBaseSongInfos(songid, out artist, out title, out numplayed, out dateadded, SourceDBPath))
                {
